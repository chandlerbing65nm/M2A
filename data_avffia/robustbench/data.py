...
def load_avffiac(
    ...
    return load_corruptions_avffia(BenchmarkDataset.avffia, n_examples,
                                  severity, data_dir, corruptions, shuffle)
...
def load_corruptions_avffia(
    ...
    if not data_root_dir.exists():
        raise DownloadError(
            f"Expected local corrupted dataset directory for AVFFIA at: {data_root_dir}. "
            "Please generate AVFFIA-C corruptions locally before calling load_avffiac."
        )
    ...
    for corruption in corruptions:
        ...
        images_all = np.load(corruption_file_path)
        images = images_all[(severity - 1) * n_total_avffia:severity *
                            n_total_avffia]
        n_img = int(np.ceil(n_examples / n_pert))
        x_test_list.append(images[:n_img])
        # Duplicate the same labels potentially multiple times
