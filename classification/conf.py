...
# Data directory
_C.DATA_DIR = "/flash/project_465002264/datasets/imagenet"
...
# Continual_MAE (CMAE) global hyperparameters
_C.block_size = 16
_C.mask_ratio = 0.5
_C.use_hog = False
_C.hog_ratio = 1.0
...
# EMA / stochastic restore / anchor-probability hyperparameters (used by CMAE-style methods)
_C.OPTIM.MT = 0.999
_C.OPTIM.RST = 0.01
_C.OPTIM.AP = 0.92
...
_C.M2A.M = 0.1                 # step size for masking ratio
_C.M2A.N = 3                   # number of masking levels
_C.M2A.LAMBDA_ERL = 1.0        # lambda for ERL term
_C.M2A.LAMBDA_EML = 1.0        # lambda for EML term
_C.M2A.MARGIN = 0.0            # margin for ERL (scaled by log(num_classes) in method)

_C.M2A.RANDOM_MASKING = "spatial"  # ['spatial', 'spectral']
_C.M2A.NUM_SQUARES = 1             # number of spatial squares for patch masking
_C.M2A.MASK_TYPE = "binary"        # ['binary', 'gaussian', 'mean']
_C.M2A.SPATIAL_TYPE = "patch"      # ['patch', 'pixel']
_C.M2A.SPECTRAL_TYPE = "all"       # ['all', 'low', 'high']

_C.M2A.SEED = 1                    # RNG seed for masking; <0 means no fixed seed

_C.M2A.DISABLE_MCL = False          # disable multi-level consistency loss
_C.M2A.DISABLE_ERL = True          # disable entropy ranking loss
_C.M2A.DISABLE_EML = False          # disable entropy minimization loss
...
# Enable random per-sample batch sizes during evaluation
_C.TEST.BATCH_RANDOM = False

# Number of full passes over all corruptions/domains and severities during evaluation
_C.TEST.LIFELONG = 1

# Number of passes with shuffled domain order (0 means disabled; >0 overrides LIFELONG)
_C.TEST.SHUFFLE_DOMAIN = 0

# Continual domain curriculum (CDC) mode: reorder batches across domains using a curriculum
...
# Save a small set of analysis images per domain (most/least confident correct/incorrect predictions)
_C.TEST.SAVE_ANALYSIS_IMAGES = False

# Save adapted model checkpoint after evaluation
_C.TEST.SAVE_CKPT = False

# Domain generalization mode: adapt on first 10 corruptions, then evaluate on remaining 5 without further adaptation
...
def assert_and_infer_cfg():
    ...
    assert _C.MODEL.ADAPTATION in ["source", "norm", "tent", "m2a", "rem", "cmae"]
...
def load_cfg_from_args(description="Config options."):
    ...
    # Optional flag to enable random per-sample batch sizes during evaluation
    parser.add_argument("--batch_random", dest="batch_random", action="store_true",
                        help="Enable random per-sample batch sizes during evaluation")
    # Optional flag to enable domain generalization mode
    parser.add_argument("--domain_gen", dest="domain_gen", action="store_true",
    ...
    # Optional flag to evaluate using a continual domain curriculum (CDC)
    parser.add_argument("--enable_cdc", dest="enable_cdc", action="store_true",
    ...
    # Optional flag to run multiple passes with shuffled domain order
    parser.add_argument("--shuffle_domain", dest="shuffle_domain", type=int, default=0,
                        help="Number of passes with shuffled domain order (0 disables shuffling)")
    # Optional flag to repeat full evaluation over all domains/severities multiple times
    parser.add_argument("--lifelong", dest="lifelong", type=int, default=1,
                        help="Number of full passes over all domains and severities during evaluation")
    # Optional flag to save a small set of analysis images (most/least confident correct/incorrect predictions)
    parser.add_argument("--save_analysis_images", dest="save_analysis_images", action="store_true",
                        help="If set, save selected evaluation samples per domain to plots/masks for qualitative analysis")
    # Optional flag to save the adapted model checkpoint at the end of evaluation
    ...
    # Apply CLI flag for random batch sizes, if provided
    if hasattr(args, "batch_random") and args.batch_random:
        cfg.TEST.BATCH_RANDOM = True

    # Apply CLI flag for domain generalization, if provided
    if hasattr(args, "domain_gen") and args.domain_gen:
        cfg.TEST.DOMAIN_GEN = True

    # Apply CLI flag for continual domain curriculum, if provided
    if hasattr(args, "enable_cdc") and args.enable_cdc:
        cfg.TEST.ENABLE_CDC = True

    # Apply CLI option for lifelong evaluation passes, if provided
    if hasattr(args, "lifelong") and args.lifelong is not None:
        try:
            cfg.TEST.LIFELONG = max(1, int(args.lifelong))
        except Exception:
            pass

    # Apply CLI option for shuffled-domain passes, if provided
    if hasattr(args, "shuffle_domain") and args.shuffle_domain is not None:
        try:
            cfg.TEST.SHUFFLE_DOMAIN = max(0, int(args.shuffle_domain))
        except Exception:
            pass

    # Apply CLI flag for saving analysis images, if provided
    if hasattr(args, "save_analysis_images") and args.save_analysis_images:
        cfg.TEST.SAVE_ANALYSIS_IMAGES = True

    # Apply CLI flag for saving adapted model checkpoints, if provided
    if hasattr(args, "save_ckpt") and args.save_ckpt:
...
def complete_data_dir_path(data_root_dir: str, dataset_name: str):
    ...
    mapping = {"imagenet": "imagenet2012",
               ...
               "mrsffia_c": "",     # path for MRSFFIA-C corruptions (handled via robustbench.data)
