...
default_cfgs = {'vit_large_patch16_384': _cfg(
...
def vit_large_patch16_384_rem(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 384x384, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer_rem('vit_large_patch16_384', pretrained=pretrained, **model_kwargs)
    return model
...
def create_model_rem(
    ...
    with set_layer_config(scriptable=scriptable, exportable=exportable, no_jit=no_jit):
        model = vit_large_patch16_384_rem(pretrained=pretrained, **kwargs)
...
def create_model(
    ...
    with set_layer_config(scriptable=scriptable, exportable=exportable, no_jit=no_jit):
        model = vit_large_patch16_384(pretrained=pretrained, **kwargs)
...
class our_vit(VisionTransformer):
    ...
    def forward_features(self, x, mask_token=None, unmask=None, return_norm=False):
        ...
        # (e.g., upsample CIFAR10 32x32 images to 384x384 for vit_large_patch16_384).
