...
def vit_tiny_patch16_224_rem(pretrained=False, **kwargs):
    """ ViT-Tiny model (ViT-Ti/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3, **kwargs)
    model = _create_vision_transformer_rem('vit_tiny_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
...
def create_model_rem(
    ...
    with set_layer_config(scriptable=scriptable, exportable=exportable, no_jit=no_jit):
        model = vit_tiny_patch16_224_rem(pretrained=pretrained, **kwargs)
