...
def load_model(model_name: str,
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        ...
        # For ImageNet "Standard" models, rely on the model ctor only (no checkpoint)
        if dataset_ == BenchmarkDataset.imagenet and 'Standard' in model_name:
            return model.eval()

        # If gdrive_id is empty/falsey (e.g. Standard_ViT_B/L on CIFAR/Imagenet-C),
        # the model ctor is responsible for loading any pretrained weights.
        # In this case, do not attempt to download or load a .pt checkpoint.
        if not models[model_name]['gdrive_id']:
            return model.eval()
