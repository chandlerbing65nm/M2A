...
from robustbench.utils import load_model, rm_substr_from_state_dict
...
class ZeroShotCLIP(nn.Module):
    ...
    def forward(self, imgs_test, return_features=False):
        # normalize the input images in their current dtype, then cast to model dtype
        # This avoids type promotion inside the normalization step (e.g. fp16 -> fp32),
        # which would otherwise conflict with half-precision CLIP weights.
        imgs_test = self.normalize(imgs_test)
...
def get_model(cfg, num_classes: int, device: Union[str, torch.device]):
    ...
    if cfg.MODEL.USE_CLIP:
        ...
        # Special handling for Standard_ViT_B: load directly from robustbench model zoo
        if cfg.MODEL.ARCH == "Standard_ViT_B":
        ...
        # Optionally load a fine-tuning checkpoint on top of the base model
        if cfg.MODEL.CKPT_PATH:
            checkpoint = torch.load(cfg.MODEL.CKPT_PATH, map_location="cpu")
            # Follow the logic from the m2a CIFAR10-C ViT script
            ...
            if isinstance(state_dict, dict):
                # remove potential 'module.' prefixes from DDP
                state_dict = rm_substr_from_state_dict(state_dict, 'module.')

            base_model.load_state_dict(state_dict, strict=True)
            logger.info(f"Successfully loaded fine-tuning checkpoint from: {cfg.MODEL.CKPT_PATH}")
