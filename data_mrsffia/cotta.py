...
def get_tta_transforms(gaussian_std: float=0.005, soft=False, clip_inputs=False, size = 384):
...
class CoTTA(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, steps=1, episodic=False, mt_alpha=0.99, rst_m=0.1, ap=0.9, size = 384):
        super().__init__()
        self.model = model
        ...
        self.transform = get_tta_transforms(size = size)    
        self.mt = mt_alpha
        self.rst = rst_m
        self.ap = ap
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        # Handle ViT-style token-wise logits of shape (B, N_tokens, C)
        ...
        anchor_logits = _cls_logits(anchor_logits)
        ...
        # Augmentation-averaged Prediction
        # Student update: symmetric KL-based entropy between student and teacher
