...
def compute_metrics(model: nn.Module,
    ...
    with torch.no_grad():
        for b in range(n_batches):
            ...
            if y_b.ndim > 1:
                y_b_adj = y_b.argmax(dim=-1)
            else:
                y_b_adj = y_b

            preds = logits.argmax(dim=1)
            correct += (preds == y_b_adj).float().sum().item()
            total_eval += y_b_adj.shape[0]

            nll_sum += F.cross_entropy(logits, y_b_adj, reduction='sum').item()
            probs = logits.softmax(dim=1)
            confs = probs.max(dim=1).values
            ents = -(probs * probs.clamp_min(1e-12).log()).sum(dim=1)

            confs_all.append(confs.detach().cpu())
            correct_all.append((preds == y_b_adj).detach().cpu())
            ...
            if getattr(cfg.TEST, "BATCH_METRICS", False):
                ...
                batch_ece = compute_ece(confs.detach(), (preds == y_b_adj).float().detach())
