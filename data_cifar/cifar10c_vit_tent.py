        ...
        for severity in cfg.CORRUPTION.SEVERITY:
            ...
            for i_x, corruption_type in enumerate(cfg.CORRUPTION.TYPE):
                x_test, y_test = load_cifar10c(cfg.CORRUPTION.NUM_EX,
                                               ...
                                               [corruption_type])
                # Keep x_test small; upsample only the current batch slice.
            ...
            for i_xx, order_i in enumerate(domain_order):
                ...
                # Adaptation happens inside model(x_curr)
                output = model(x_curr)
    ...
    for severity in cfg.CORRUPTION.SEVERITY:
        ...
        for perm_idx in range(n_permutations):
            ...
            for pos_in_order, idx in enumerate(order):
                ...
                # Optionally save forward features/logits/probabilities/predictions/labels per corruption
...
def compute_metrics(model: torch.nn.Module,
    ...
    for i in range(n_batches):
        ...
        if not no_adapt:
            ...
            # no adaptation: forward through underlying base model only
            base_core = _unwrap_base_model_for_features(model)
            _feats, logits = _extract_features_and_logits(base_core, x_b)
        # Ensure logits batch dimension matches labels
        if logits.dim() > 2 or logits.shape[0] != y_b.shape[0]:
            try:
                b = y_b.shape[0]
                c = logits.shape[-1]
                k = logits.numel() // (b * c)
                logits = logits.view(b, k, c).mean(dim=1)
            except Exception:
                # Fallback: flatten all but batch and class dims best-effort
                logits = logits.view(y_b.shape[0], -1)[:, : logits.shape[-1]]
