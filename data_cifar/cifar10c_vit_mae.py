...
def _extract_features_logits_and_block_cls(base_model: torch.nn.Module,
    ...
    try:
        ...
        if getattr(core, 'dist_token', None) is None:
            x = torch.cat((cls_token, x), dim=1)
        else:
            x = torch.cat((cls_token, core.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
        ...
        x = core.norm(x)
        feats = core.pre_logits(x[:, 0]) if hasattr(core, 'pre_logits') else x[:, 0]
