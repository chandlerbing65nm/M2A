...
def save_cam_overlay(x: torch.Tensor, cam: np.ndarray, out_path: str, alpha: float = 0.5) -> None:
    ...
    x_np = np.transpose(x_np, (1, 2, 0))       # [H, W, C]
    x_np = np.clip(x_np, 0.0, 1.0)
    img = (x_np * 255.0).astype(np.uint8)

    # Bright color CAM using inferno colormap
    cam_map = np.clip(cam, 0.0, 1.0)
    cmap = cm.get_cmap("inferno")
    cam_color = cmap(cam_map)[..., :3]         # [H, W, 3] in [0,1]
    heat = (cam_color * 255.0).astype(np.float32)

    # Blend image and CAM: image weight = 1 - alpha, CAM weight = alpha
    overlay = img.astype(np.float32) * (1.0 - alpha) + heat * alpha
    overlay = np.clip(overlay, 0.0, 255.0).astype(np.uint8)

    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    Image.fromarray(overlay).save(out_path, format="JPEG")
...
def save_cam_only(cam: np.ndarray, out_path: str) -> None:
    ...
    ax.imshow(cam_map, cmap="inferno", interpolation="bilinear")
    ax.axis("off")
